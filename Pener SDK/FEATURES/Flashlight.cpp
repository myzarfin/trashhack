...
{
	static DWORD oConstructor = UTILS::FindSignature("client.dll", "55 8B EC F3 0F 10 45 ? B8");
...
{
	static DWORD oDestructor = UTILS::FindSignature("client.dll", "56 8B F1 E8 ? ? ? ? 8B 4E 28");
	__asm
	{
		mov ecx, pFlashLight
		push ecx
...
void CFlashLight::UpdateFlashLight(CFlashLightEffect *pFlashLight, const Vector &vecPos, const Vector &vecForward, const Vector &vecRight, const Vector &vecUp)
{
	...
	if (!oUpdateLight)
	{
		DWORD callInstruction = UTILS::FindSignature("client.dll", "E8 ? ? ? ? 8B 06 F3 0F 10 46"); // get the instruction address
